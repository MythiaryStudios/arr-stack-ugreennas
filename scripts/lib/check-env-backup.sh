...
check_env_backup() {
    ...
    if ! timeout 2 ping -c 1 moosenas.local &>/dev/null && ! timeout 2 ping -c 1 192.168.0.136 &>/dev/null; then
    ...
    if [[ -n "$NAS_SSH_PASS" ]] && command -v sshpass &>/dev/null; then
        nas_env=$(timeout 10 sshpass -p "$NAS_SSH_PASS" ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 mooseadmin@moosenas.local "cat /volume1/docker/arr-stack/.env" 2>/dev/null)
    else
        nas_env=$(timeout 10 ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 -o BatchMode=yes mooseadmin@moosenas.local "cat /volume1/docker/arr-stack/.env" 2>/dev/null)
    fi
    ...
    if [[ "$nas_env" != "$local_env" ]]; then
        ...
        echo "             Run: scp mooseadmin@moosenas.local:/volume1/docker/arr-stack/.env .env.nas.backup"
